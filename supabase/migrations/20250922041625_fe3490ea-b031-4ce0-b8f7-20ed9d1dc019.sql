-- Force RLS and secure all views (idempotent migration)
...
-- Move extensions to dedicated schema if relocatable
CREATE SCHEMA IF NOT EXISTS extensions;
DO $$ 
DECLARE e record; 
BEGIN
  FOR e IN SELECT e.extname, e.oid FROM pg_extension e WHERE e.extnamespace='public'::regnamespace
  LOOP 
    IF (SELECT extrelocatable FROM pg_extension WHERE oid=e.oid) THEN
