-- Update unified dashboard to fetch 30 days instead of 14 for better historic data
-- This supports the 4-week brand presence rate chart

-- Drop both function overloads
DROP FUNCTION IF EXISTS public.get_unified_dashboard_data(uuid);
DROP FUNCTION IF EXISTS public.get_unified_dashboard_data(uuid, uuid);
...
BEGIN
  ...
  -- CHANGED: Fetch 30 days instead of 14 to support 4-week charts
  ...
  responses_data AS (
    ...
    WHERE ppr.org_id = p_org_id
      AND ppr.run_at >= now() - interval '30 days'  -- CHANGED from 14 to 30 days
      AND (p_brand_id IS NULL OR ppr.brand_id = p_brand_id OR ppr.brand_id IS NULL)
    ORDER BY ppr.run_at DESC
    LIMIT 1000  -- Increased limit for 30 days of data
  ),
  chart_data AS (
    SELECT 
...
GRANT EXECUTE ON FUNCTION public.get_unified_dashboard_data(uuid, uuid) TO authenticated;

COMMENT ON FUNCTION public.get_unified_dashboard_data IS 'Returns unified dashboard data with 30 days of historic data to support 4-week trend charts';