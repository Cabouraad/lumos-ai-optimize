-- Update get_low_visibility_prompts to consider 'completed' responses (not just 'success')
-- This fixes the issue where recommendations return 0 due to status mismatch
...
BEGIN
  ...
  WITH recent_responses AS (
    ...
    WHERE ppr.org_id = v_org_id
      AND ppr.status IN ('success', 'completed') -- CHANGED HERE
  ...
  prompt_stats AS (
    ...
    HAVING COUNT(*) >= 1  -- include prompts with at least 1 run (allows 0% presence)
  )
  SELECT
    p.id AS prompt_id,
