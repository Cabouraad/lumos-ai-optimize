-- Add brand_id column to tables for brand-specific filtering
ALTER TABLE public.prompts ADD COLUMN IF NOT EXISTS brand_id UUID REFERENCES public.brands(id) ON DELETE SET NULL;
ALTER TABLE public.prompt_provider_responses ADD COLUMN IF NOT EXISTS brand_id UUID REFERENCES public.brands(id) ON DELETE SET NULL;
ALTER TABLE public.optimizations_v2 ADD COLUMN IF NOT EXISTS brand_id UUID REFERENCES public.brands(id) ON DELETE SET NULL;
ALTER TABLE public.recommendations ADD COLUMN IF NOT EXISTS brand_id UUID REFERENCES public.brands(id) ON DELETE SET NULL;

-- Create indexes for brand filtering
CREATE INDEX IF NOT EXISTS idx_prompts_brand_id ON public.prompts(brand_id);
CREATE INDEX IF NOT EXISTS idx_prompt_provider_responses_brand_id ON public.prompt_provider_responses(brand_id);
CREATE INDEX IF NOT EXISTS idx_optimizations_v2_brand_id ON public.optimizations_v2(brand_id);
CREATE INDEX IF NOT EXISTS idx_recommendations_brand_id ON public.recommendations(brand_id);

-- Update the RPC function to support brand filtering
