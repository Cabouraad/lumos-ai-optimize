-- Fix get_org_competitor_summary_v2 to properly filter brand_catalog by brand_id
-- This ensures multi-brand accounts see separate competitor lists per brand

CREATE OR REPLACE FUNCTION get_org_competitor_summary_v2(
...
BEGIN
  ...
  -- CRITICAL: Filter brand_catalog by brand_id for proper multi-brand isolation
  valid_expanded AS (
    SELECT e.*
    FROM expanded e
    ...
    -- Also check exclusions to filter out excluded competitors
