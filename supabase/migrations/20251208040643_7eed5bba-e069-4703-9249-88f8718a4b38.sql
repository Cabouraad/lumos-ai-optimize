-- Update compute_llumos_score to support brand filtering
...
BEGIN
  ...
  -- Count total responses with brand filtering
  SELECT COUNT(*)
  ...
  ELSE
    -- 1. PR: Presence Rate (with brand filtering)
    SELECT 
      COALESCE(
    ...
    -- 2. PP: Prominence Position (with brand filtering)
    SELECT 
      COALESCE(
        100 - LEAST(
    ...
    -- 3. CV: Coverage Variance (with brand filtering)
    IF v_scope = 'org' THEN
    ...
    -- 5. CS: Competitive Share (with brand filtering)
    ...
    -- 6. FC: Freshness & Consistency (with brand filtering)
...
-- Clear recent cached scores so fresh brand-specific scores are computed
DELETE FROM llumos_scores WHERE created_at > NOW() - INTERVAL '24 hours';