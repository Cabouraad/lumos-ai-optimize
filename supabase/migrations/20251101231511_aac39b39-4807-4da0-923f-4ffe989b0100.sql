-- Update compute_llumos_score RPC with CA Integration
...
BEGIN
  ...
  ELSE
    ...
    -- 4. CA: Citation Authority (NEW - conditional)
    IF v_enable_ca THEN
    ...
    -- Composite calculation with conditional CA weight
    IF v_enable_ca THEN
      -- Original weights: restored when CA is enabled
      v_composite := (
        v_pr * 0.25 +  -- Presence Rate
        v_pp * 0.20 +  -- Prominence Position
        v_cv * 0.15 +  -- Coverage Variance
        v_ca * 0.20 +  -- Citation Authority (RESTORED)
        v_cs * 0.15 +  -- Competitive Share
        v_fc * 0.05    -- Freshness & Consistency
      );
      RAISE NOTICE '[LlumosScore] Composite = % (WITH CA)', ROUND(v_composite, 2);
    ELSE
      -- Current weights: CA disabled, redistributed
      v_composite := (
        v_pr * 0.30 +  -- +5%
        v_pp * 0.24 +  -- +4%
        v_cv * 0.18 +  -- +3%
        v_ca * 0.00 +  -- 0% (disabled)
        v_cs * 0.18 +  -- +3%
        v_fc * 0.10    -- +5%
      );
      RAISE NOTICE '[LlumosScore] Composite = % (WITHOUT CA)', ROUND(v_composite, 2);
    END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'ca_enabled', v_enable_ca
  );
END;
$$;

COMMENT ON FUNCTION public.compute_llumos_score IS 'Computes Llumos Score with conditional Citation Authority (CA) scoring based on org feature flag';