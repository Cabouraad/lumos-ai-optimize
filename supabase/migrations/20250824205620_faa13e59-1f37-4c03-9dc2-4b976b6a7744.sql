-- Phase 2: Database Cleanup and Optimization (Fixed)
...
-- Add composite indexes for common query patterns (without CONCURRENTLY)
CREATE INDEX IF NOT EXISTS idx_ppr_org_prompt_run_at 
ON prompt_provider_responses (org_id, prompt_id, run_at DESC);

CREATE INDEX IF NOT EXISTS idx_ppr_org_status_run_at 
ON prompt_provider_responses (org_id, status, run_at DESC) 
WHERE status = 'success';

-- Optimize competitor_mentions for better performance
CREATE INDEX IF NOT EXISTS idx_competitor_mentions_org_prompt 
ON competitor_mentions (org_id, prompt_id, normalized_name);

-- Add index for brand_catalog performance
CREATE INDEX IF NOT EXISTS idx_brand_catalog_org_type 
