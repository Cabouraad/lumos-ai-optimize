-- Update get_low_visibility_prompts to support brand filtering
CREATE OR REPLACE FUNCTION public.get_low_visibility_prompts(
  p_org_id uuid DEFAULT NULL::uuid, 
  p_limit integer DEFAULT 20,
  p_brand_id uuid DEFAULT NULL::uuid
)
 RETURNS TABLE(prompt_id uuid, prompt_text text, total_runs bigint, presence_rate numeric, avg_score_when_present numeric, last_checked_at timestamp with time zone, top_citations jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WITH recent_responses AS (
    ...
    WHERE ppr.org_id = v_org_id
      ...
      -- Brand filter: only include responses for prompts matching brand
  ...
  WHERE p.org_id = v_org_id
    ...
    -- Also filter prompts by brand_id
    AND (p_brand_id IS NULL OR p.brand_id = p_brand_id)
