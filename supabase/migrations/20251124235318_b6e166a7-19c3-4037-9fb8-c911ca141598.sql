-- Fix competitor exclusions to work permanently
-- Update get_org_competitor_summary_v2 to respect org_competitor_exclusions table
...
BEGIN
  ...
  -- CRITICAL FIX: Filter out excluded competitors
  excluded_competitors AS (
    SELECT LOWER(TRIM(competitor_name)) AS excluded_name
  ...
  valid_expanded AS (
    ...
    -- CRITICAL: Exclude competitors in exclusions table
    AND e.competitor_name_lower NOT IN (SELECT excluded_name FROM excluded_competitors)
  ),
  aggregated AS (
    SELECT
