-- Update compute_llumos_score RPC to support brand filtering
-- This allows calculating Llumos scores for specific brands within an organization
...
DECLARE
  ...
  v_reason TEXT;
BEGIN
  -- Set window (last 14 days)
  v_window_end := NOW();
  v_window_start := NOW() - INTERVAL '14 days';
  ...
  v_pp := LEAST(100, v_avg_prominence * 10);
  ...
  WITH recent_activity AS (
    ...
    INNER JOIN prompts p ON p.id = ppr.prompt_id
    WHERE ppr.org_id = p_org_id
      AND ppr.run_at >= NOW() - INTERVAL '3 days'
      AND ppr.status = 'success'
      AND (p_prompt_id IS NULL OR ppr.prompt_id = p_prompt_id)
      AND (p_brand_id IS NULL OR p.brand_id = p_brand_id)
  )
  ...
  v_composite := (
    ...
    v_ca * 0.15 +  -- Citation authority
    v_cs * 0.10 +  -- Competitive share
    v_fc * 0.10    -- Freshness
  ) / 100;

  -- Scale to 300-850 range (like credit score)
  v_score := ROUND(300 + (v_composite * 550));
