-- Fix ambiguous column reference in get_org_competitor_summary_v2
-- The issue is with column naming conflicts in the final SELECT
...
BEGIN
  ...
  -- Get list of excluded competitors
  excluded_competitors AS (
    SELECT LOWER(TRIM(oce.competitor_name)) AS excluded_name
  ...
  -- Only keep names that exist in brand_catalog AND are not excluded
  valid_expanded AS (
    SELECT e.id, e.org_id, e.run_at, e.prompt_id, e.score, e.competitor_name_lower
    FROM expanded e
    ...
    -- Exclude competitors in exclusions table
    AND e.competitor_name_lower NOT IN (SELECT ec.excluded_name FROM excluded_competitors ec)
  ),
  aggregated AS (
    SELECT
      ve.competitor_name_lower AS competitor_key,
      COUNT(*)::bigint AS total_mentions,
      COUNT(DISTINCT ve.prompt_id)::bigint AS distinct_prompts,
      MIN(ve.run_at) AS first_seen,
      MAX(ve.run_at) AS last_seen,
      AVG(ve.score)::numeric AS avg_score
    FROM valid_expanded ve
    GROUP BY ve.competitor_name_lower
  ),
  totals AS (
  ...
  with_trend AS (
    SELECT
      ve.competitor_name_lower AS competitor_key,
      CASE
        WHEN COUNT(CASE WHEN ve.run_at >= (CURRENT_TIMESTAMP - interval '7 days') THEN 1 END) > 0
         AND COUNT(CASE WHEN ve.run_at < (CURRENT_TIMESTAMP - interval '7 days') THEN 1 END) > 0
        THEN
          ((COUNT(CASE WHEN ve.run_at >= (CURRENT_TIMESTAMP - interval '7 days') THEN 1 END)::numeric /
            NULLIF(COUNT(CASE WHEN ve.run_at < (CURRENT_TIMESTAMP - interval '7 days') THEN 1 END)::numeric, 0)) - 1) * 100
        ELSE 0
      END AS trend_score
    FROM valid_expanded ve
    GROUP BY ve.competitor_name_lower
  ...
  SELECT
    ...
    ) AS result_competitor_name,
    agg.total_mentions AS result_total_mentions,
    agg.distinct_prompts AS result_distinct_prompts,
    agg.first_seen AS result_first_seen,
    agg.last_seen AS result_last_seen,
    agg.avg_score AS result_avg_score,
    CASE
      WHEN tot.grand_total > 0 THEN ((agg.total_mentions::numeric / tot.grand_total) * 100)
      ELSE 0
    END AS result_share_pct,
    COALESCE(wt.trend_score, 0)::numeric AS result_trend_score
  FROM aggregated agg
...
COMMENT ON FUNCTION public.get_org_competitor_summary_v2 IS 'Returns competitor summary for an organization, filtering out excluded competitors from org_competitor_exclusions table';