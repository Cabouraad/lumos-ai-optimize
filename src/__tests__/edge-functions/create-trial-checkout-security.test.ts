...
describe('create-trial-checkout security tests', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('should include idempotency key in Stripe call', async () => {
    ...
    const key = generateIdempotencyKey('user123', 'trial-checkout');
    
    expect(key).toMatch(/^user123:trial-checkout:\d+$/);
  ...
  it('should generate consistent idempotency keys within time window', () => {
    ...
    const key1 = generateIdempotencyKey('user123', 'trial-checkout');
    const key2 = generateIdempotencyKey('user123', 'trial-checkout');
  ...
  it('should generate different keys for different users', () => {
    ...
    const key1 = generateIdempotencyKey('user123', 'trial-checkout');
    const key2 = generateIdempotencyKey('user456', 'trial-checkout');
